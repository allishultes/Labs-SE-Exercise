...
describe('Reversioning script', () => {
  ...
  it('findShot: should return proper object', () => {
    ...
    const firstResult = reversion.findShot(testScript.script[0], firstTestIndex);
    expect(firstResult).to.deep.equal(testScript.script[0].shots[0]);
    const secondResult = reversion.findShot(testScript.script[0], secondTestIndex);
    expect(secondResult).to.deep.equal(testScript.script[0].shots[1]);
  });
});
